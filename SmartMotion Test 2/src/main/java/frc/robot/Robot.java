...
import frc.utilities.Setpoints;
...
public class Robot extends TimedRobot {
  ...
  public void teleopPeriodic() {
    ...
    // display current POV to SmartDashboard
    SmartDashboard.putNumber("POV", xboxController.getPOV());

    // set pov to angle from dpad
    double pov = SmartDashboard.getNumber("POV", -1);

    /* if setpoints.dpad is false, set mode to false for 'setpoint'; otherwise is joystick on xbox controller
    is moved set to true for 'velocity' */
    if (!Setpoints.dpad(pov)){
      SmartDashboard.putBoolean("Mode", false);
    } else if ((Robot.xboxController.getRawAxis(Xbox.LAxisY) < -0.2) || (Robot.xboxController.getRawAxis(Xbox.LAxisY) > 0.2)){
